...
def draw_smooth_line(image, pt1, pt2, color, thickness, alpha):
    overlay = image.copy()
...
def apply_filter(input_path, output_path, min_length, max_length, allowed_error, opacity):
    ...
    if height > 450 or width > 600:
        ...
        np_image = cv2.resize(image, (width, height), interpolation=cv2.INTER_LINEAR)
...
#getting our variables set up for drawing lines in gradient directions
    ...
    for brush_radius in brushes:
        ...
        min = min_length
        max = max_length
        line_width = int(random.choice(range(int(min), int(max+1))))

        if len(strokes1) != 0:
            strokes1 = np.array(strokes1)
            strokes2 = np.array(strokes2)
            step_size = line_width * brush_radius
            x1 = strokes1[:, 0]
            y1 = strokes1[:, 1]
            grad_dirs = strokes2
            
            normal_dir = grad_dirs + np.pi/2 #randomize + or - pi/2 can be good, but we choose not to

            dx = (step_size/2) * np.cos(normal_dir)
            dy = (step_size/2) * np.sin(normal_dir)
            ...
            strokes1_with_endpoints = np.hstack([strokes1, x2[:, np.newaxis], y2[:, np.newaxis], x3[:, np.newaxis], y3[:, np.newaxis]]) #adding to strokes list
            ...
            for i in indices:
                ...
                draw_smooth_line(np_image2, (int(strokes1_with_endpoints[i, 4]), int(strokes1_with_endpoints[i, 5])), (int(strokes1_with_endpoints[i, 2]), int(strokes1_with_endpoints[i, 3])), color, brush_radius,opacity)

        np_image3 = np_image2
    cv2.imwrite(output_path, cv2.cvtColor(np_image2, cv2.COLOR_RGB2BGR))
...
#                 stroke_length_range = (min_length, max_length)
#             else:
#                 stroke_length_range = (max_length, min_length)
